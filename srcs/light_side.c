...
int			side_light_check(t_dot origin, t_dot inter, t_dot inter2, t_obj *o, t_sec_r s, t_data *d)
{
	float	d1;
	float	d2;
...
int			call_side_light_check(t_sec_r s, t_obj *obj, t_data *d)
{
	...
	ret = side_light_check(org, d1, d2, obj, s, d);
	org = d->cam->pos;
	...
	if (ret == 0 || ret != side_light_check(org, d1, d2, obj, s, d))
		return (-1);
	return (1);
}
